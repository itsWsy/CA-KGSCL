    ...
    def _prepare_additional_data(self):
        ...
        if self.config.model_type.lower() == 'knowledge':
            if self.config.kg_data_type == 'pretrain':
                ...
                pass
        if self.model_name in ['DuoRec', 'DuoRec2', 'UniContraRec', 'DuoRec_Backbone']:
        ...
        elif self.model_name in ['KGSCL', 'KGSCL_2', 'SASRec_KGAUG', 'KGSCL_Backbone']:
            kg_relation_dict = load_pickle(os.path.join(self.data_path, 'kg_relational_set.pkl'))
                ...
                if len(complement_item_list) > 0:
                    for sub_item in complement_item_list:
                        ...
                        if co_item_pair not in co_occurrence_all_data.keys():
                            ...
                            complement_freq_list.append(1. + float(co_occurrence_all_data[co_item_pair]))
        ...
        with open(file_path, 'r') as fr:
            for line in fr.readlines():
                ...
                if len(item_seq) < self.filter_len:
                    continue
                if len(item_seq) < self.min_seq_len or len(item_seq) > self.max_seq_len:
                    continue
