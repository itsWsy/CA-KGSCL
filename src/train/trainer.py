...
class Trainer:
    ...
    def pretraining(self):
        ...
        if self.pretraining_task in ['MISP', 'MIM', 'PID']:
            ...
            pretrain_dataset = pretrain_dataset(self.num_items, self.config, self.data_dict['train'], self.additional_data_dict)
        else:
    ...
    def training(self):
        ...
        train_dataset = SpecifiedDataSet(self.num_items, self.config, self.data_dict['train'], self.additional_data_dict)
        ...
        eval_dataset = SpecifiedDataSet(self.num_items, self.config, self.data_dict['eval'],
        ...
        for epoch in range(self.epoch_num):
            ...
            for i, batch in train_iter:
                # training forward
                batch = (t.to(self.dev) for t in batch)
                loss = self.training_model.train_forward(*batch, epoch=epoch)
    ...
    def _load_sequential_model(self):
        ...
        specified_seq_model = Model(self.num_items, self.config).to(self.dev)
        return specified_seq_model

    def _load_model_with_additional_data(self):
        Model = getattr(model, self.model_name)
        specified_model = Model(self.num_items, self.config, self.additional_data_dict).to(self.dev)
    ...
    def _set_num_items(self):
        ...
        self.config.max_item = self.data_processor.max_item
    ...
    def test_model(self, test_data_pair=None):
        SpecifiedDataSet = load_specified_dataset(self.model_name, self.config)
        test_dataset = SpecifiedDataSet(self.num_items, self.config, test_data_pair,
                                        self.additional_data_dict, train=False)
