...
class Reorder(object):
    ...
    def __call__(self, sequence):
        ...
        start_index = random.randint(0, len(copied_sequence) - sub_seq_len - 1)
...
class Repeat(object):
    ...
    def __call__(self, sequence):
        # make a deep copy to avoid original sequence be modified
        copied_sequence = copy.deepcopy(sequence)
        max_repeat_nums = int(self.p * len(copied_sequence))
        repeat_nums = \
...
AUGMENTATIONS = {'crop': Crop, 'mask': Mask, 'reorder': Reorder, 'repeat': Repeat}
